...
export function StatsDashboard({ source, kpi, userKpi = {}, charts }: StatsDashboardProps) {
  ...
  const [topDaysToShow, setTopDaysToShow] = useState(31)

  const currentMonthKey = useMemo(() => {
    const dates = publicDailyTrends
  ...
  const monthDailyOrders = useMemo(() => {
    if (!currentMonthKey) return []
    return publicDailyTrends.filter((row) => String(row.date ?? "").startsWith(currentMonthKey))
  }, [publicDailyTrends, currentMonthKey])

  const sliderMax = Math.max(3, monthDailyOrders.length || 3)
  const sliderValue = Math.min(Math.max(topDaysToShow, 3), sliderMax)

  const topDaysForItemHeatmap = useMemo(() => {
    const sorted = [...monthDailyOrders]
      .sort((a, b) => toNumber(b.transactions) - toNumber(a.transactions))
      .slice(0, sliderValue)
      .sort((a, b) => String(a.date).localeCompare(String(b.date)))
      .map((row) => String(row.date))
    return sorted
  }, [monthDailyOrders, sliderValue])

  const topItemsForHeatmap = useMemo(() => {
    if (transactionTopItems.length > 0) {
      return transactionTopItems.slice(0, 8).map((row) => row.item_name)
    }
  ...
  const itemDayLookup = useMemo(() => {
    const lookup = new Map<string, number>()
    for (const row of transactionItemHeatmap) {
      lookup.set(`${row.item_name}__${row.date}`, row.order_count)
    }
    return lookup
  }, [transactionItemHeatmap])

  const itemHeatmapMax = useMemo(() => {
    let max = 0
    for (const itemName of topItemsForHeatmap) {
      for (const day of topDaysForItemHeatmap) {
        max = Math.max(max, itemDayLookup.get(`${itemName}__${day}`) ?? 0)
      }
    }
    return Math.max(max, 1)
  }, [topItemsForHeatmap, topDaysForItemHeatmap, itemDayLookup])

  const hasItemMonthlyHeatmap = topItemsForHeatmap.length > 0 && topDaysForItemHeatmap.length > 0
  const heatmapDays = [...topDaysForItemHeatmap].sort()
    ...
    <div className="space-y-4">
      ...
      <Tabs defaultValue="overview" className="space-y-4">
        ...
        <TabsContent value="heatmaps" className="space-y-4">
                          ...
                          {Array.from({ length: 24 }).map((_, hour) => {
                            ...
                            return (
                              <div
                                ...
                                className="h-5 rounded-sm border border-border/20"
          ...
          <Card className="border-border/50">
            <CardHeader className="pb-3">
              <CardTitle className="text-sm font-medium">Items les plus commandes x jours les plus charges</CardTitle>
            </CardHeader>
            <CardContent className="space-y-3">
              <div className="space-y-1">
                <div className="flex items-center justify-between text-xs text-muted-foreground">
                  <span>Jours affiches (tries par volume)</span>
                  <span>{sliderValue} / {sliderMax}</span>
                </div>
                <input
                  type="range"
                  min={3}
                  max={sliderMax}
                  value={sliderValue}
                  onChange={(event) => setTopDaysToShow(Number(event.target.value))}
                  className="w-full"
                />
              </div>

              {hasItemMonthlyHeatmap ? (
                <div className="overflow-x-auto">
                  <div className="min-w-[860px] space-y-2">
                    <div
                      className="grid items-center gap-1"
                      style={{ gridTemplateColumns: `180px repeat(${heatmapDays.length}, minmax(24px, 1fr))` }}
                    >
                      <p className="text-[10px] text-muted-foreground">Item</p>
                      {heatmapDays.map((date) => (
                        <p key={date} className="text-center text-[10px] text-muted-foreground" title={date}>
                          {date.slice(8, 10)}
                        </p>
                      ))}
                    </div>
                    {topItemsForHeatmap.map((itemName) => (
                      <div
                        key={itemName}
                        className="grid items-center gap-1"
                        style={{ gridTemplateColumns: `180px repeat(${heatmapDays.length}, minmax(24px, 1fr))` }}
                      >
                        <p className="truncate pr-2 text-[11px] text-muted-foreground" title={itemName}>
                          {itemName}
                        </p>
                        {heatmapDays.map((date) => {
                          const count = itemDayLookup.get(`${itemName}__${date}`) ?? 0
                          return (
                            <div
                              key={`${itemName}-${date}`}
                              className="h-6 rounded-sm border border-border/30"
                              style={{ backgroundColor: heatColor(count, itemHeatmapMax) }}
                              title={`${itemName} • ${date}: ${count}`}
                            />
                          )
                        })}
                      </div>
                ...
                <div className="flex h-24 items-center justify-center text-sm text-muted-foreground">
                  Pas assez de donnees itemisees pour cette heatmap.
                </div>
              )}
              ...
              <div className="flex items-center gap-2 text-[10px] text-muted-foreground">
                ...
                <span>0</span>
                <div className="h-2 w-16 rounded-sm" style={{ backgroundColor: heatColor(itemHeatmapMax * 0.5, itemHeatmapMax) }} />
                <span>moyen</span>
                <div className="h-2 w-16 rounded-sm" style={{ backgroundColor: heatColor(itemHeatmapMax, itemHeatmapMax) }} />
                <span>fort</span>
              </div>
            </CardContent>
          </Card>
