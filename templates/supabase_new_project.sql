...
COMMENT ON SCHEMA audit IS 'Schéma d''audit sécurisé. Accès réservé uniquement au rôle postgres. Seules les fonctions SECURITY DEFINER peuvent accéder aux tables et fonctions.';
...
COMMENT ON TABLE private.transactions IS 'RLS activé. Lecture via vues, écriture via RPC SECURITY DEFINER uniquement.';
...
CREATE VIEW dashboard_view.non_members WITH (security_invoker='on') AS
 SELECT id,
    ...
    created_at
   FROM private.users u
  WHERE ((is_active = true) AND (role <> 'membre'::public.user_role) AND (role <> ALL (ARRAY['administrateur'::public.user_role, 'superadmin'::public.user_role, 'marketing'::public.user_role, 'caissier'::public.user_role])));
...
COMMENT ON VIEW dashboard_view.polls IS 'Vue des sondages avec options et nombre de votes. Accès uniquement via la fonction RPC get_dashboard_boot_data() qui vérifie le rôle administrateur/marketing. L''accès direct est désactivé pour des raisons de sécurité.';
...
CREATE TRIGGER auth_user_created_private AFTER INSERT ON auth.users FOR EACH ROW EXECUTE FUNCTION private.handle_new_user();



CREATE TRIGGER detect_auth_anomaly_trigger AFTER INSERT ON auth.audit_log_entries FOR EACH ROW EXECUTE FUNCTION audit.trigger_detect_auth_anomaly();



CREATE TRIGGER tr_block_disposable_emails BEFORE INSERT ON auth.users FOR EACH ROW EXECUTE FUNCTION private.check_disposable_email();
...
ALTER TABLE ONLY audit.security_anomalies
    ADD CONSTRAINT security_anomalies_resolved_by_fkey FOREIGN KEY (resolved_by) REFERENCES auth.users(id);



ALTER TABLE ONLY audit.security_anomalies
    ADD CONSTRAINT security_anomalies_user_id_fkey FOREIGN KEY (user_id) REFERENCES auth.users(id);
...
ALTER TABLE ONLY private.app_access_stats
    ADD CONSTRAINT app_access_stats_user_id_fkey FOREIGN KEY (user_id) REFERENCES private.users(id) ON DELETE CASCADE;



ALTER TABLE ONLY private.notification_tokens
    ADD CONSTRAINT notification_tokens_v2_user_id_fkey FOREIGN KEY (user_id) REFERENCES auth.users(id) ON DELETE CASCADE;
...
ALTER TABLE ONLY public.section_visibility
    ADD CONSTRAINT section_visibility_updated_by_fkey FOREIGN KEY (updated_by) REFERENCES auth.users(id);
...
ALTER TABLE audit.security_anomalies ENABLE ROW LEVEL SECURITY;
...
CREATE POLICY allow_authenticated_listen ON realtime.messages FOR SELECT USING (((( SELECT auth.role() AS role) = 'authenticated'::text) AND (topic = ('user-'::text || ( SELECT auth.email() AS email)))));



CREATE POLICY allow_user_email_channel_push ON realtime.messages FOR INSERT WITH CHECK (((( SELECT auth.role() AS role) = 'authenticated'::text) AND (topic = ('user-'::text || ( SELECT auth.email() AS email)))));
...
CREATE POLICY "TEMP_DIAGNOSTIC_ALLOW_ALL" ON storage.objects TO authenticated USING (true) WITH CHECK (true);
...
CREATE PUBLICATION transactions_pub WITH (publish = 'insert, update, delete, truncate');



ALTER PUBLICATION supabase_realtime ADD TABLE ONLY private.transactions;



ALTER PUBLICATION transactions_pub ADD TABLE ONLY private.transactions;
...
REFRESH MATERIALIZED VIEW public.mv_faq;
